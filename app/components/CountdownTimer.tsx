...
export default function CountdownTimer() {
  ...
  // Reset timeLeft when initialTimerMinutes changes, but only if not running
  useEffect(() => {
    if (!isRunning) {
      setTimeLeft(initialTimerMinutes * 60);
    }
  }, [initialTimerMinutes, isRunning]);
  ...
  useEffect(() => {
    if (isRunning && timeLeft > 0) {
      ...
      Vibration.vibrate([500, 500, 500]); // Vibrate when timer ends
      // Optional: You could play a sound or show a notification here
    }

    // Cleanup function: Clear interval when component unmounts or dependencies change
    return () => clearInterval(timerIntervalRef.current);
  }, [isRunning, timeLeft]); // Dependencies for the effect

  // Timer control functions
  const handleStartStop = () => {
    if (isRunning) {
      // If running, stop and reset to initial time
      clearInterval(timerIntervalRef.current);
      setIsRunning(false);
      setTimeLeft(initialTimerMinutes * 60);
    } else {
  ...
  const handleAdjustTime = (amountInMinutes) => {
    if (!isRunning) { // Only allow adjustments when the timer is not running
      setInitialTimerMinutes((prev) => {
        const newMinutes = prev + amountInMinutes;
        if (newMinutes < minMinutes) return minMinutes; // Don't go below minimum
        if (newMinutes > maxMinutes) return maxMinutes; // Don't exceed maximum
        return newMinutes;
      });
    }
  };
  ...
  return (
    <View style={styles.timerSection}>
        <View style={styles.timerCircleContainer}>
          <Text variant="headlineSmall" style={styles.headline}>Meditation Timer</Text>
          {/* Minus Button */}
          <IconButton
            icon="minus-circle"
            size={50}
        ...
        <Button
          ...
          icon={isRunning ? "stop" : "play"} // Change icon based on timer state
        >
